 ...
 * 时间：2016/5/10
 */
public abstract class NineGridLayout3 extends ViewGroup {
...
//        mSingleWidth = (int) ((mTotalWidth - mSpacing * (3 - 1)) / 3);
        mSingleWidth = DensityUtils.dip2px(mContext, 200f);
        if (mIsFirst) {
    ...
    private ImageView createDeleteImageView(final int i, final String url) {
        ImageView imageView = new ImageView(mContext);
        imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
        imageView.setImageResource(R.drawable.image_show_piceker_del);
    ...
    private void layoutImageView(RatioImageView imageView, int i, String url, boolean showNumFlag) {
        final int singleWidth = (int) ((mTotalWidth - mSpacing * (3 - 1)));
        int singleHeight = mSingleWidth;
        ...
        if (!TextUtils.isEmpty(url)) {
            ImageView deleteIv = createDeleteImageView(i, url);
            int left1 = right - DensityUtils.dip2px(mContext, 25f);
            int top1 = top;
            int right1 = right;
            int bottom1 = top1 + DensityUtils.dip2px(mContext, 25f);
            deleteIv.layout(left1, top1, right1, bottom1);
            addView(deleteIv);
        }
