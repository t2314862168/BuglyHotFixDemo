 ...
 */
public class PtrFrameLayoutEx extends ViewGroup {
    ...
    private PtrUIHandlerHolderEx mPtrUIHandlerHolder = PtrUIHandlerHolderEx.create();
    private PtrHandlerEx mPtrHandler;
    // working parameters
    ...
    private PtrIndicatorEx mPtrIndicator;
    ...
    public PtrFrameLayoutEx(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPtrIndicator = new PtrIndicatorEx();
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        boolean isInStartPosition = mPtrIndicator.isInStartPosition();
        // 由于在ACTION_DOWN的时候会调用BGABadgeViewHelper的onTouchEvent方法,这里我们先自己拦截;有问题再说
        if (action == MotionEvent.ACTION_DOWN && !isInStartPosition) {
            return true;
        }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if ((moveUp && canMoveUp) || (moveDown && mPullDistance <= MAX_HEADER_PULL_HEIGHT)) {
                    movePos(offsetY);
                    return true;
                }
        }
     ...
     * 设置头部最大下拉距离
     *
     * @param maxHeaderPullHeight
     */
    public void setMaxHeaderPullHeight(int maxHeaderPullHeight) {
        MAX_HEADER_PULL_HEIGHT = maxHeaderPullHeight;
    }
    ...
    protected void onPositionChange(boolean isInTouching, byte status, PtrIndicatorEx mPtrIndicator) {
    }
    ...
    public void setPtrHandler(PtrHandlerEx ptrHandler) {
        mPtrHandler = ptrHandler;
    }

    public void addPtrUIHandler(PtrUIHandlerEx ptrUIHandler) {
        PtrUIHandlerHolderEx.addHandler(mPtrUIHandlerHolder, ptrUIHandler);
    }

    @SuppressWarnings({"unused"})
    public void removePtrUIHandler(PtrUIHandlerEx ptrUIHandler) {
        mPtrUIHandlerHolder = PtrUIHandlerHolderEx.removeHandler(mPtrUIHandlerHolder, ptrUIHandler);
    }

    public void setPtrIndicator(PtrIndicatorEx slider) {
